...
TRAILER_ALLOW = IDENTITY_ALLOW + (re.compile(r"^noreply@anthropic\.com$", re.IGNORECASE),)
...
def main():
    ...
    out = subprocess.run(
        ["git", "log", f"--format=%H{FIELD_SEP}%ae{FIELD_SEP}%ce{FIELD_SEP}%B{RECORD_SEP}", f"{base}..{head}"],
        capture_output=True,
        text=True,
        check=True,
    ).stdout
    ...
    for record in records:
        ...
        for label, email in (("author", author_email), ("committer", committer_email)):
            if not allowed(email, IDENTITY_ALLOW):
                failures.append(f"{sha[:12]}: {label} email {email!r} is not @gmail.com or a GitHub noreply address")

        for email in TRAILER_RE.findall(body):
            if not allowed(email, TRAILER_ALLOW):
                failures.append(f"{sha[:12]}: Co-authored-by trailer references disallowed email {email!r}")
